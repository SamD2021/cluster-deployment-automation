...
"""
            ...
            if not apply_yaml_content(self.client, pvc_yaml, f"PVC '{pvc_name}'"):
...
class HostPathStorage(ClusterStorage):
    ...
    def deploy_storage(self) -> None:
        ...
        # Apply storage class
        if not apply_yaml_content(self.client, storage_class_yaml, "storage class"):
    ...
    def create_pv_with_node_affinity(self, pv_name: str, storage_size: str, storage_path: Optional[str] = None) -> None:
        ...
        # Apply the PV
        if not apply_yaml_content(self.client, rendered, f"PV '{pv_name}' with node affinity"):
    ...
    def _apply_daemonset_with_template(self) -> None:
        ...
        # Apply the DaemonSet
        if not apply_yaml_content(self.client, rendered, "storage directory creator DaemonSet"):
