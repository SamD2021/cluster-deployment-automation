...
def ExtraConfigImageRegistry(cc: ClustersConfig, cfg: ExtraConfigArgs, futures: dict[str, Future[Optional[host.Result]]]) -> None:
    ...
    match cfg.registry_type:
        ...
        case RegistryType.MICROSHIFT.value:
            ...
            if registry_node.container_name:
                lh = host.MincHost(registry_node.container_name)
            else:
                lh = host.LocalHost()
            # Set up storage configuration for MicroShift
            registry_storage_size = registry_node.in_cluster_registry_storage_size
            microshift_storage = create_cluster_storage(kubeconfig_path=cc.kubeconfig, storage_type=StorageType.HOSTPATH, target_node_hostname=registry_node.name, storage_path="/var/lib/registry-storage")

            # Create MicroShift registry instance
            microshift_reg = MicroshiftRegistry(host=lh, kubeconfig=cc.kubeconfig, external_ip=registry_node.ip, storage=microshift_storage, storage_size=registry_storage_size)

            logger.info("Redeploying MicroShift registry...")
            # Undeploy existing registry and storage first
            microshift_reg.undeploy()
            microshift_storage.undeploy_storage()

            # Deploy storage foundation first
            ...
            microshift_storage.deploy_storage()
