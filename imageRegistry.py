...
"""
        ...
        # Create new PVC
        if not apply_yaml_content(self.client, pvc_manifest, "registry PVC"):
...
class MicroshiftRegistry(InClusterRegistry):
    def __init__(self, host: host.Host, kubeconfig: str, external_ip: str, storage: ClusterStorage, base_dns_domain: str = "nip.io", storage_size: str = "8Gi") -> None:
    ...
    def deploy(self) -> None:
        ...
        self.storage._ensure_storage_directory()
        self.storage.create_pv_with_node_affinity(pv_name="registry-pv", storage_size=self.storage_size, storage_path="/var/lib/registry-storage")
        self._create_registry_pvc()
    ...
    def _configure_microshift_registry(self) -> None:
        ...
        if not apply_yaml_content(self.client, rendered_manifest, "registry manifest"):
